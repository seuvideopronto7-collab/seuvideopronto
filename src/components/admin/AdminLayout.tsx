...
const AdminLayout = ({ title, description, children, actionLabel, onAction }: AdminLayoutProps) => {
  ...
  const menu = useMemo(
    () => [
      { label: "Visao Geral", path: "/admin/overview", icon: LayoutGrid },
      { label: "Geracao de Video", path: "/admin/video-generator", icon: Film },
      { label: "Renderizacoes", path: "/admin/renders", icon: Gauge },
      { label: "Roteiros", path: "/admin/scripts", icon: ShieldCheck },
      { label: "Vozes / Narracao", path: "/admin/voices", icon: Users },
      { label: "Trilhas Sonoras", path: "/admin/soundtracks", icon: Film },
      { label: "Integracoes API", path: "/admin/integrations", icon: Settings },
      { label: "Publicacao Social", path: "/admin/social-publishing", icon: Bell },
          ...
          <header className="sticky top-0 z-40 border-b border-border/50 bg-card/60 backdrop-blur">
            <div className="px-4 lg:px-8 py-4 flex flex-wrap items-center gap-3 justify-between">
              ...
              <div className="flex flex-1 items-center justify-end gap-3">
                ...
                <div className="status-pill status-processing">PROCESSANDO</div>
